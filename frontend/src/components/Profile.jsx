...
const Profile = () => {
 const params = useParams()
  const {data} = useGetProfileQuery(params._id)
  console.log(data);
  ...
  const isLoggedInUserProfile = userInfo?._id === data?._id;
  ...
  const displayedPost = activeTab === 'posts' ? data?.posts: data?.bookmarks;
          ...
          <section>
            <div className='flex flex-col gap-5'>
              ...
              <div className='flex flex-col gap-1'>
                <span className='font-semibold'>{data?.bio || 'bio here...'}</span>
                <Badge className='w-fit' variant='secondary'><AtSign /> <span className='pl-1'>{data?.username}</span> </Badge>
